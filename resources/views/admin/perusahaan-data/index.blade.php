...
<div class="space-y-6">
    <div class="flex justify-between items-center">
        <h2 class="text-2xl font-bold text-gray-800">🏢 Data Perusahaan PKL Siswa</h2>
    ...
    <div class="flex gap-2 border-b border-gray-200 pb-2">
        <a href="{{ route('admin.perusahaan-data') }}" class="px-4 py-2 text-sm font-medium rounded-t-lg {{ request('filter') !== 'approved' ? 'bg-blue-100 text-blue-800' : 'text-gray-600 hover:text-gray-800' }}">
            Semua ({{ \App\Models\PerusahaanData::count() }})
        </a>
        <a href="{{ route('admin.perusahaan-data') }}?filter=approved" class="px-4 py-2 text-sm font-medium rounded-t-lg {{ request('filter') === 'approved' ? 'bg-green-100 text-green-800' : 'text-gray-600 hover:text-gray-800' }}">
            ✅ Approved ({{ \App\Models\PerusahaanData::where('is_verified', true)->count() }})
        </a>
        <a href="{{ route('admin.perusahaan-data') }}?filter=pending" class="px-4 py-2 text-sm font-medium rounded-t-lg {{ request('filter') === 'pending' ? 'bg-yellow-100 text-yellow-800' : 'text-gray-600 hover:text-gray-800' }}">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm">
                ...
                <tbody class="divide-y">
                    @forelse($data as $d)
                    <tr class="hover:bg-gray-50 {{ !$d->is_verified ? 'bg-yellow-50/30' : '' }}">
			    ...
			    <div class="text-xs text-gray-500 line-clamp-1">{{ $d->alamat_pembimbing }}</div>
                            <div class="text-xs text-gray-500">{{ $d->ttl_pembimbing }}</div>
                        </td>
                            ...
                            <div class="flex justify-end gap-1">
                                ...
                                @else
                                    ...
                                    <form action="{{ route('admin.perusahaan-data.reject', $d) }}" method="POST" onsubmit="return confirm('Batalkan approval untuk data ini?')">
                                        @csrf
                                        <button type="submit" class="text-gray-400 hover:text-gray-600 p-1.5 rounded hover:bg-gray-50 transition text-xs" title="Batalkan">
                                            ↺
                                        </button>
                                    </form>
                                @endif
        ...
        <div class="px-4 py-3 border-t bg-gray-50">{{ $data->links() }}</div>
    </div>
</div>
@endsection
