...
<div class="max-w-3xl mx-auto space-y-6">
    <div class="flex items-center gap-3">
        ...
        <h2 class="text-2xl font-bold text-white drop-shadow-md">🏢 Input Kunjungan Industri</h2>
    </div>
    ...
    <div class="glass-panel p-6 rounded-xl shadow-[0_0_15px_rgba(0,0,0,0.1)] border border-white/5">
        <form action="{{ route('pembimbing.kunjungan.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            ...
            <div>
                <label class="block text-sm font-medium text-gray-300 mb-1">Pilih Perusahaan Binaan</label>
            ...
            <div id="rencana-container" class="space-y-4">
                <div>
                    ...
                    <textarea name="catatan_rencana" id="catatan-rencana-input" rows="4" 
                              ...
                              class="w-full bg-crypto-dark border border-white/20 text-white placeholder-gray-500 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-crypto-accent focus:border-crypto-accent transition-colors">{{ old('catatan_rencana') }}</textarea>
                </div>
            </div>
            ...
            <div id="selesai-container" class="space-y-4 hidden">
                <div>
                    ...
                    <textarea name="catatan" id="catatan-input" rows="4" 
                              ...
                              class="w-full bg-crypto-dark border border-white/20 text-white placeholder-gray-500 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-crypto-accent focus:border-crypto-accent transition-colors">{{ old('catatan') }}</textarea>
                </div>
            ...
            <div class="flex justify-end gap-3 pt-4 border-t border-white/10">
                ...
                <button type="submit" id="kunjungan_create_submit" class="px-6 py-2.5 bg-crypto-success hover:bg-emerald-500 text-white rounded-lg font-bold transition-colors flex items-center gap-2 shadow-[0_0_15px_rgba(14,203,129,0.3)] active:scale-95">
